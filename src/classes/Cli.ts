...
class Cli {
  ...
  createVehicle(): void {
    inquirer
      ...
      .then((answers) => {
        // create a truck or motorbike if the user selects the respective vehicle type
  ...
  findVehicleToTow(truck: Truck): void { // adds parameter to accept a truck object
    inquirer
      .prompt([
        {
          type: 'list',
      ...
      .then((answers) => {
        ...
        if (selectedVehicle.vin === truck.vin) {
          ...
          truck.tow(answers.vehicleToTow); // tow the selected vehicle
          this.performActions();
        }
      });
  }
    ...
    inquirer
      ...
      .then((answers) => {
        ...
        // perform the wheelie action only if the selected vehicle is a motorbike
          ...
          // return to the initial prompt if the user wants to select or create another vehicle
          this.startCli();
          return;
        } else {
      ...
      .prompt([
        {
          ...
          when: () => this.vehicles.length > 0 || true,
        }
      ])
      .then((answers) => {
        ...
        if (answers.CreateOrSelect === 'Create a new vehicle') {
          ...
          if (this.vehicles.length === 0) {
            console.log('No vehicles have been created yet.');
            this.createVehicle();
          } else {
            this.chooseVehicle();
          }
